...
import { ArrowLeft, Save, Church, MapPin, Phone, Mail, Image as ImageIcon, FileText, Globe } from "lucide-react";
...
export default function NovaComunidadePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Erro ao criar comunidade");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-parish-text">Nova Comunidade</h1>
            <p className="text-parish-text-light mt-1">Adicione uma nova comunidade à paróquia</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-parish-surface rounded-lg shadow-sm border border-parish-primary p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" name="name" value={formData.name} onChange={handleChange} required placeholder="Ex: São Sebastião - Igreja Matriz" className="w-full px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none" />
              <p className="text-xs text-parish-secondary mt-1">Nome completo da comunidade/igreja</p>
            </div>
            <div>
              ...
              <textarea name="description" value={formData.description} onChange={handleChange} rows={4} placeholder="Descreva a comunidade, sua história, patrono..." className="w-full px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none resize-none" />
            </div>
            <div>
              ...
              <p className="text-xs text-parish-secondary mt-1">Menor número aparece primeiro (0 = primeira)</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-parish-surface rounded-lg shadow-sm border border-parish-primary p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input type="text" name="address" value={formData.address} onChange={handleChange} placeholder="Rua, número" className="w-full px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-parish-text-light mb-2">Bairro</label>
              <input type="text" name="neighborhood" value={formData.neighborhood} onChange={handleChange} placeholder="Ex: Três Barras" className="w-full px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-parish-text-light mb-2">CEP</label>
              <input type="text" name="zipCode" value={formData.zipCode} onChange={handleChange} placeholder="00000-000" className="w-full px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none" />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <input type="url" name="mapUrl" value={formData.mapUrl} onChange={handleChange} placeholder="https://maps.google.com/..." className="w-full px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-parish-surface rounded-lg shadow-sm border border-parish-primary p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="(65) 0000-0000" className="w-full px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-parish-text-light mb-2">
                <div className="flex items-center space-x-2"><Mail className="w-4 h-4" /><span>Email</span></div>
              </label>
              <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="comunidade@paroquia.com.br" className="w-full px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-parish-surface rounded-lg shadow-sm border border-parish-primary p-6">
          ...
          <div>
            ...
            <input type="url" name="image" value={formData.image} onChange={handleChange} placeholder="https://exemplo.com/imagem.jpg" className="w-full px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none" />
            <p className="text-xs text-parish-secondary mt-1">Cole a URL de uma imagem da comunidade/igreja</p>
          </div>
          {formData.image && (
            <div className="mt-4">
              <p className="text-sm font-medium text-parish-text-light mb-2">Preview:</p>
              <img src={formData.image} alt="Preview" className="w-full max-w-md h-48 object-cover rounded-lg" onError={(e) => { (e.target as HTMLImageElement).src = ""; toast.error("Erro ao carregar imagem"); }} />
            </div>
          )}
        </div>
        ...
        <div className="bg-parish-surface rounded-lg shadow-sm border border-parish-primary p-6">
          ...
          <p className="text-xs text-parish-secondary mt-2">Apenas comunidades ativas serão exibidas no site público</p>
        </div>
        ...
        <div className="flex items-center justify-end space-x-4">
          ...
          <button type="submit" disabled={loading} className="px-6 py-2 bg-parish-gold text-white rounded-lg hover:bg-parish-gold-dark transition flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed">
            {loading ? (
              ...
              <><Save className="w-4 h-4" /><span>Criar Comunidade</span></>
            )}
          </button>
        </div>
      </form>
