...
import { ArrowLeft, Save, Clock, Calendar, Church, FileText } from "lucide-react";
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (activeCommunities.length > 0) {
          setFormData((prev) => ({ ...prev, communityId: activeCommunities[0].id }));
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/missas", {
        ...
        body: JSON.stringify({ ...formData, observations: formData.observations.trim() || null }),
      });
      if (!response.ok) {
      ...
      toast.error(error instanceof Error ? error.message : "Erro ao criar missa");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-3xl font-bold text-parish-text">Nova Missa</h1>
        </div>
        <div className="bg-parish-sky-light border border-parish-sky-light rounded-lg p-6 text-center">
          <Church className="w-12 h-12 text-parish-sky-dark mx-auto mb-4" />
          <h3 className="text-lg font-semibold text-parish-text mb-2">Nenhuma comunidade cadastrada</h3>
          <p className="text-parish-text-light mb-4">Você precisa cadastrar pelo menos uma comunidade antes de criar missas.</p>
          <Link href="/admin/comunidades/nova" className="inline-flex items-center px-4 py-2 bg-parish-gold text-white rounded-lg hover:bg-parish-gold-dark transition">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-parish-text">Nova Missa</h1>
            <p className="text-parish-text-light mt-1">Adicione um novo horário de celebração</p>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-parish-text-light mb-2">
                <div className="flex items-center space-x-2"><Church className="w-4 h-4" /><span>Comunidade *</span></div>
              </label>
              <select name="communityId" value={formData.communityId} onChange={handleChange} required className="w-full px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none">
                <option value="">Selecione uma comunidade</option>
                {communities.map((community) => (
                  <option key={community.id} value={community.id}>
                    {community.name}{community.neighborhood && ` - ${community.neighborhood}`}
                  </option>
                ))}
              </select>
              <p className="text-xs text-parish-secondary mt-1">Selecione a igreja/comunidade onde a missa será celebrada</p>
            </div>
        ...
        <div className="flex items-center justify-end space-x-4">
          ...
          <button type="submit" disabled={loading} className="px-6 py-2 bg-parish-gold text-white rounded-lg hover:bg-parish-gold-dark transition flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed">
            {loading ? (
              ...
              <><Save className="w-4 h-4" /><span>Criar Missa</span></>
            )}
          </button>
        </div>
      </form>
