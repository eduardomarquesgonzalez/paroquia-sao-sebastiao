...
export default function NovoSacramentoPage() {
  ...
  const addDocument = () => setRequiredDocuments((prev) => [...prev, ""]);
  const updateDocument = (i: number, val: string) =>
    setRequiredDocuments((prev) => prev.map((d, idx) => idx === i ? val : d));
  const removeDocument = (i: number) =>
    setRequiredDocuments((prev) => prev.filter((_, idx) => idx !== i));
  ...
  const handleSubmit = async (active: boolean) => {
    ...
    try {
      const res = await fetch("/api/sacramentos", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          requiredDocuments: requiredDocuments.filter((d) => d.trim()),
        }),
      });
      if (!res.ok) {
      ...
      toast.success("Sacramento cadastrado com sucesso!");
      router.push("/admin/sacramentos");
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/admin/sacramentos" className="text-parish-text-light hover:text-parish-text transition">
            <ArrowLeft className="w-6 h-6" />
          </Link>
          <div>
            <h1 className="text-3xl font-bold text-parish-text">Novo Sacramento</h1>
            <p className="text-parish-text-light mt-1">Cadastre as informações do sacramento</p>
          </div>
        </div>
        <div className="flex gap-3">
          <button
            ...
            className="px-4 py-2 border border-parish-border rounded-lg text-parish-text-light hover:bg-parish-background transition disabled:opacity-50 flex items-center gap-2 text-sm"
          >
            <Save className="w-4 h-4" /> Salvar Inativo
          </button>
          <button
          ...
          >
            {isSubmitting ? "Salvando..." : "Salvar e Ativar"}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-parish-surface rounded-lg shadow-sm p-6 border border-parish-primary space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-parish-text-light mb-2">Nome do Sacramento *</label>
              <input
                type="text" name="name" value={formData.name} onChange={handleChange}
                placeholder="Ex: Batismo, Matrimônio, Eucaristia..."
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Descreva o sacramento, sua importância e como funciona na paróquia..."
            ...
            <div>
              ...
              <input
                type="text" name="responsible" value={formData.responsible} onChange={handleChange}
                placeholder="Padre, diácono ou coordenador responsável"
              ...
              {schedules.map((s, i) => (
                <div key={i} className="flex gap-2">
                  <input
                    ...
                    placeholder="Ex: Sábados às 10h, mediante agendamento"
          ...
          <div className="bg-parish-surface rounded-lg shadow-sm p-6 border border-parish-primary">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-parish-text">Documentos Necessários</h2>
              <button type="button" onClick={addDocument} className="text-parish-gold hover:text-parish-gold-dark text-sm flex items-center gap-1 font-medium transition">
                <Plus className="w-4 h-4" /> Adicionar
              </button>
            </div>
            <div className="space-y-3">
              {requiredDocuments.map((d, i) => (
                <div key={i} className="flex gap-2">
                  <input
                    type="text" value={d} onChange={(e) => updateDocument(i, e.target.value)}
                    placeholder="Ex: Certidão de nascimento, RG..."
                    className="flex-1 px-4 py-2 border border-parish-border rounded-lg focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none text-sm"
                  />
                  {requiredDocuments.length > 1 && (
                    <button type="button" onClick={() => removeDocument(i)} className="text-red-400 hover:text-red-600 transition p-2">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  )}
                </div>
