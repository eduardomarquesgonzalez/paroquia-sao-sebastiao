...
import { Plus, Search, Edit, Trash2, Eye, EyeOff, Sparkles, Star } from "lucide-react";
...
export default function SacramentosAdminPage() {
  ...
  async function fetchItems() {
    try {
      setLoading(true);
      const res = await fetch("/api/sacramentos?all=1");
      if (!res.ok) throw new Error("Erro ao carregar");
      setItems(await res.json());
    } catch {
      toast.error("Erro ao carregar sacramentos");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleDelete(id: string) {
    if (!confirm("Tem certeza que deseja excluir este sacramento?")) return;
    try {
      const res = await fetch(`/api/sacramentos?id=${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
      toast.success("Sacramento excluído!");
      setItems((prev) => prev.filter((i) => i.id !== id));
    } catch {
      toast.error("Erro ao excluir sacramento");
    }
  }

  async function toggleActive(item: SacramentInfo) {
    try {
      ...
      toast.success(item.active ? "Sacramento desativado" : "Sacramento ativado");
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-parish-text">Sacramentos</h1>
          <p className="text-parish-text-light mt-1">Gerencie as informações dos sacramentos</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <Plus className="w-5 h-5" /> Novo Sacramento
          </Link>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-parish-surface rounded-lg p-12 text-center border border-parish-primary">
          <Sparkles className="w-16 h-16 text-parish-secondary mx-auto mb-4" />
          <p className="text-parish-text-light mb-2">Nenhum sacramento encontrado</p>
          <Link href="/admin/sacramentos/novo" className="text-parish-gold hover:text-parish-gold-dark text-sm">
            Cadastrar o primeiro sacramento
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((item) => (
            ...
            >
              {/* Image */}
              <div className="h-40 bg-gradient-to-br from-parish-navy/20 to-parish-sky/10 flex-shrink-0 overflow-hidden relative">
                {item.image ? (
                  <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center">
                    <Sparkles className="w-10 h-10 text-parish-gold/40" />
                  </div>
                )}
                {item.highlight && (
              ...
              <div className="p-4">
                ...
                {item.schedules.length > 0 && (
                  <p className="text-xs text-parish-text-light mb-1 line-clamp-1">
                    📅 {item.schedules[0]}{item.schedules.length > 1 ? ` +${item.schedules.length - 1}` : ""}
                  </p>
                )}
                {item.responsible && (
                  <p className="text-xs text-parish-text-light mb-3 line-clamp-1">👤 {item.responsible}</p>
                )}

                <div className="flex items-center justify-between pt-3 border-t border-parish-primary">
                  <Link
                    href={`/admin/sacramentos/${item.id}/editar`}
