          ...
          {loadingEventos ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {eventos.map((evento, index) => {
                const isEnded = isEventEnded(evento);
                const firstUpcomingIdx = eventos.findIndex((e) => !isEventEnded(e));
                return (
                  <Link
                    key={evento.id}
                    href={`/eventos/${evento.id}`}
                    className={`group flex flex-col bg-parish-surface rounded-2xl overflow-hidden border transition-all duration-300 animate-on-scroll ${
                      index === 1 ? "stagger-1" : index === 2 ? "stagger-2" : ""
                    } ${
                      isEnded
                        ? "border-parish-border/40 opacity-70 hover:opacity-80 hover:shadow-sm"
                        : "border-parish-border/70 hover:shadow-navy hover:-translate-y-2"
                    }`}
                  >
                    {/* Image */}
                    <div className="relative h-52 overflow-hidden flex-shrink-0">
                      {evento.image ? (
                        ...
                        <div className={`w-full h-full flex items-center justify-center ${isEnded ? "bg-gradient-to-br from-gray-300/50 to-gray-400/30" : "bg-gradient-navy"}`}>
                          <Calendar className={`w-14 h-14 ${isEnded ? "text-gray-400/50" : "text-white/25"}`} />
                        </div>
                      )}
                      <div className={`absolute inset-0 bg-gradient-to-t ${isEnded ? "from-black/65 via-transparent to-transparent" : "from-parish-navy-dark/55 via-transparent to-transparent"}`} />
                    ...
                    <div className="flex-1 flex flex-col p-6">
                      <h3 className={`font-playfair text-lg font-bold line-clamp-2 mb-2 leading-snug transition-colors duration-200 ${
                        isEnded
                          ? "text-parish-text-light group-hover:text-parish-text"
                          : "text-parish-navy-dark group-hover:text-parish-gold"
                      }`}>
                        {evento.title}
                      </h3>
                      ...
                      <div className="space-y-2 mt-auto">
                        <div className="flex items-center gap-2 text-xs text-parish-text-light">
                          <div className={`w-6 h-6 rounded-md flex items-center justify-center flex-shrink-0 ${isEnded ? "bg-gray-100" : "bg-parish-gold/10"}`}>
                            <Clock className={`w-3.5 h-3.5 ${isEnded ? "text-gray-400" : "text-parish-gold"}`} />
                          </div>
                        ...
                        {evento.location && (
                          <div className="flex items-center gap-2 text-xs text-parish-text-light">
                            <div className={`w-6 h-6 rounded-md flex items-center justify-center flex-shrink-0 ${isEnded ? "bg-gray-100" : "bg-parish-gold/10"}`}>
                              <MapPin className={`w-3.5 h-3.5 ${isEnded ? "text-gray-400" : "text-parish-gold"}`} />
                            </div>
                      ...
                      <div className="mt-5 pt-4 border-t border-parish-border flex items-center justify-between">
                        <span className={`text-sm font-semibold flex items-center gap-1.5 transition-all duration-200 ${
                          isEnded ? "text-gray-400 group-hover:gap-2" : "text-parish-gold group-hover:gap-2.5"
                        }`}>
                          {isEnded ? "Ver registro" : "Saiba mais"}
                          <ArrowRight className="w-3.5 h-3.5" />
                        </span>
                        <span className={`text-xs px-2.5 py-1 rounded-full ${isEnded ? "text-gray-400 bg-gray-100" : "text-parish-text-light bg-parish-background"}`}>
                          {getEventMonth(evento.date)}
                        </span>
                      </div>
                    </div>
