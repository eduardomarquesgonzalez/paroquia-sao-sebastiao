...
export default function SacramentosPage() {
  const [items, setItems] = useState<SacramentInfo[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/sacramentos")
      .then((r) => r.json())
  ...
  return (
    <div className="min-h-screen bg-parish-background">
      ...
      <section className="relative bg-parish-navy-dark overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-parish-gold/8 rounded-full -translate-y-1/2 translate-x-1/3 blur-3xl pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-parish-sky/10 rounded-full translate-y-1/2 -translate-x-1/3 blur-3xl pointer-events-none" />
        ...
        <div className="container mx-auto px-4 py-24 relative">
          <nav className="flex items-center gap-2 text-xs text-white/45 mb-10">
            ...
            <span className="text-white/70">Sacramentos</span>
          </nav>

          <div className="max-w-2xl">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-11 h-11 rounded-xl bg-parish-gold/20 border border-parish-gold/30 flex items-center justify-center">
                <Sparkles className="w-5 h-5 text-parish-gold" />
              </div>
            ...
            <p className="text-lg text-white/65 leading-relaxed max-w-lg">
              Os sacramentos são sinais eficazes da graça instituídos por Cristo.
              Conheça os sacramentos celebrados em nossa paróquia e como participar.
            </p>
          </div>
          ...
          {!loading && items.length > 0 && (
            <div className="mt-12 flex flex-wrap gap-4">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl px-5 py-3 border border-white/10">
                ...
                <p className="text-xs text-white/55 mt-0.5">Sacramentos disponíveis</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-lg text-parish-text-light leading-relaxed italic">
              &ldquo;Os sacramentos da nova lei, instituídos por nosso Senhor Jesus Cristo,
              contêm a graça que significam e a conferem a quem os recebe dignamente.&rdquo;
            </p>
            <p className="text-sm text-parish-gold font-semibold mt-4">— Catecismo da Igreja Católica</p>
          </div>
        </div>
      </section>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-parish-surface rounded-3xl border border-parish-border">
              <div className="w-16 h-16 rounded-2xl bg-parish-gold/10 flex items-center justify-center mx-auto mb-4">
                <Sparkles className="w-8 h-8 text-parish-gold" />
              </div>
              <p className="font-semibold text-parish-text mb-1">Informações em breve</p>
              <p className="text-sm text-parish-text-light">Os sacramentos serão cadastrados em breve</p>
            </div>
          ) : (
            <>
              {/* Highlighted */}
                ...
                <div className="mb-12">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {highlighted.map((item) => (
                      <SacramentCard key={item.id} item={item} featured />
                    ))}
                  </div>
                </div>
              )}
      ...
      <section className="py-16 bg-parish-surface border-t border-parish-border">
        <div className="container mx-auto px-4 text-center">
          <h2 className="font-playfair text-2xl font-bold text-parish-navy-dark mb-3">Quer receber um sacramento?</h2>
          <p className="text-parish-text-light mb-6 max-w-md mx-auto text-sm leading-relaxed">
            Entre em contato com a secretaria paroquial para agendar ou obter mais informações.
          </p>
          <Link
            href="/contato"
...
function SacramentCard({ item, featured = false }: { item: SacramentInfo; featured?: boolean }) {
  return (
    <article className={`group bg-parish-surface rounded-2xl overflow-hidden border border-parish-border/60 shadow-sm hover:shadow-navy hover:-translate-y-2 transition-all duration-300 flex flex-col ${featured ? "ring-1 ring-parish-gold/30" : ""}`}>
      {/* Image */}
      ...
      <div className="flex-1 flex flex-col p-6">
        ...
        <div className="space-y-2.5 mt-auto mb-5">
          ...
          {item.requiredDocuments.length > 0 && (
            <div className="flex items-start gap-2.5 text-xs text-parish-text-light">
              <div className="w-5 h-5 rounded bg-parish-gold/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                <FileText className="w-3 h-3 text-parish-gold" />
              </div>
              <div>
                <p className="font-medium text-parish-text mb-1">Documentos necessários:</p>
                <ul className="space-y-0.5 list-disc list-inside">
                  {item.requiredDocuments.slice(0, 3).map((doc, i) => (
                    <li key={i}>{doc}</li>
                  ))}
                  {item.requiredDocuments.length > 3 && (
                    <li className="text-parish-gold">+{item.requiredDocuments.length - 3} documentos</li>
                  )}
                </ul>
              </div>
            </div>
