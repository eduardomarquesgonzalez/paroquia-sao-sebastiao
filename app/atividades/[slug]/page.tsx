...
function CampoDinamico({
  ...
  const base =
    "w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-parish-gold focus:border-transparent outline-none text-gray-800 bg-white transition text-sm"
    ...
    case "SELECT":
      return (
        ...
        >
          ...
          {campo.opcoes.filter(Boolean).map((op) => (
    ...
    case "CHECKBOX":
      return (
        <div className="space-y-2">
          {campo.opcoes.filter(Boolean).map((op) => {
            const checked = Array.isArray(value) ? value.includes(op) : false
            return (
              <label key={op} className="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    onChange(e.target.checked ? [...prev, op] : prev.filter((v) => v !== op))
                  }}
                  className="w-4 h-4 text-parish-gold border-gray-300 rounded focus:ring-parish-gold"
                />
          ...
          {campo.opcoes.filter(Boolean).map((op) => (
            <label key={op} className="flex items-center gap-3 cursor-pointer group">
              <input
                ...
                required={campo.obrigatorio}
                className="w-4 h-4 text-parish-gold border-gray-300 focus:ring-parish-gold"
              />
              <span className="text-sm text-gray-700 group-hover:text-gray-900">{op}</span>
            </label>
          ))}
    ...
    case "TELEFONE":
      return (
        <input
          ...
          value={value as string}
          onChange={(e) => onChange(e.target.value)}
          placeholder={campo.placeholder ?? "(65) 9 9999-9999"}
          required={campo.obrigatorio}
          className={base}
        />
      )
...
export default function AtividadePublicaPage() {
  ...
  const loadAtividade = useCallback(async () => {
    try {
      const res = await fetch(`/api/atividades/${slug}`)
      if (res.status === 404) { setNotFound(true); return }
      if (!res.ok) throw new Error()
      const data: Atividade = await res.json()
      if (!data.active) { setNotFound(true); return }
      setAtividade(data)
      // Inicializar respostas
      if (data.formulario?.campos) {
        const init: Record<string, string | string[]> = {}
        data.formulario.campos.forEach((c) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErroEnvio("")
    try {
      // Extrair nome/email/telefone das respostas para campos de topo
      ...
      setSucesso(true)
      setShowForm(false)
      // Recarregar para atualizar contagem de vagas
      loadAtividade()
    } catch {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <Loader2 className="w-10 h-10 text-parish-gold animate-spin" />
        </div>
      </div>
    )
  }

  if (notFound || !atividade) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        <PublicNavbar />
        <div className="flex-1 flex flex-col items-center justify-center gap-4 px-4">
          ...
          <p className="text-gray-500 text-center">Esta atividade não existe ou não está disponível.</p>
          <Link href="/" className="mt-2 px-5 py-2.5 bg-parish-gold text-white rounded-xl font-semibold hover:bg-parish-gold-dark transition">
            Voltar ao início
          </Link>
        </div>
        <PublicFooter />
  ...
  const vagasStatus = getVagasStatus(atividade.formulario)
  const inscricoesAbertas = isInscricoesAbertas(atividade.formulario)
